                ...
                <div class="card-footer">
                    <?php 
                        if(isset($_POST['sub'])){
                            ...
                            if($new == $confirm ){
                                $q = mysqli_query($con,"select * from student where S_Id = '$sid' and S_pass = '$old'");
                                if ( mysqli_fetch_assoc($q)){
                                    mysqli_query($con,"update student set S_pass ='$new' where S_Id = '$sid' "); 
                            ?>
                                <h6 class="text-center"> Password Changed Successfully!!!</h6>
                            <?php
                                }
                                else {
                                    header("location:student_change_password.php?oldp=dinvalid");
                                }
                            }
                            else {
                                header("location:student_change_password.php?invalid=didnot_match");
                            }
...
<div class="text-center mt-5">
        <a href="student_change_password.php?logout=log" class="btn-lg btn-danger">LogOut</a>
</div>
