                ...
                <div class="card-footer">
                    <?php 
                        if(isset($_POST['sub'])){
                            ...
                            if($new == $confirm ){
                                $q = mysqli_query($con,"select * from teacher where T_Id = '$tid' and pass = '$old'");
                                if ( mysqli_fetch_assoc($q)){
                                    mysqli_query($con,"update teacher set pass ='$new' where T_Id = '$tid' "); 
                            ?>
                                <h6 class="text-center"> Password Changed Successfully!!!</h6>
                            <?php
                                }
                                else {
                                    header("location:teacher_change_password.php?oldp=dinvalid");
                                }
                            }
                            else {
                                header("location:teacher_change_password.php?invalid=didnot_match");
                            }
